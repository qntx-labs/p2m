...
pub fn detect_tables_from_lines(items: &[TextItem], lines: &[Line], page: u32) -> Vec<Table> {
    // Filter lines for this page
    let page_lines: Vec<&Line> = lines.iter().filter(|l| l.page.get() == page).collect();
    ...
    let page_item_count = items.iter().filter(|i| i.page.get() == page).count();
...
mod tests {
    ...
    fn make_hline(y: f32, x1: f32, x2: f32, page: u32) -> Line {
        Line {
            x1,
            y1: y,
            x2,
    ...
    fn make_vline(x: f32, y1: f32, y2: f32, page: u32) -> Line {
        Line {
            x1: x,
            y1,
            x2: x,
